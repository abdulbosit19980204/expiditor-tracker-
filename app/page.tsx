...
export default function ExpeditorTracker() {
  ...
  // Filter and sort expeditors by today's checks
  const filteredExpeditors = Array.isArray(expeditors)
    ? expeditors
        .filter((expeditor) => {
          if (!expeditorSearchQuery) return true
        ...
        .sort((a, b) => getTodayChecksCount(b.name) - getTodayChecksCount(a.name))
    : []
  ...
  const SidebarContent = () => (
    <div className="h-full flex flex-col">
      <div className="p-4 border-b border-gray-200">
        ...
        <div className="mb-4">
          <label className="text-sm font-medium text-gray-700 mb-2 block">Sana oralig'i</label>
          <DatePickerWithRange
            dateRange={filters.dateRange}
            onDateRangeChange={(range) =>
              setFilters((prev) => ({ ...prev, dateRange: range || { from: undefined, to: undefined } }))
        ...
        <div className="mb-4">
          ...
          {isFiltersOpen && (
            <div className="mt-3 space-y-3 p-3 bg-gray-50 rounded-lg">
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Loyiha</label>
                <Select
                  value={filters.project}
                  onValueChange={(value) => setFilters((prev) => ({ ...prev, project: value === "all" ? "" : value }))}
                >
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Ombor</label>
                <Select
                  value={filters.sklad}
                  onValueChange={(value) => setFilters((prev) => ({ ...prev, sklad: value === "all" ? "" : value }))}
                >
              ...
              <div>
                <label className="text-xs font-medium text-gray-600 mb-1 block">Shahar</label>
                <Select
                  value={filters.city}
                  onValueChange={(value) => setFilters((prev) => ({ ...prev, city: value === "all" ? "" : value }))}
                >
        ...
        <div className="relative">
          <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
          <Input
            placeholder="Ekspeditorlarni qidirish..."
            value={expeditorSearchQuery}
            onChange={(e) => setExpeditorSearchQuery(e.target.value)}
            className="pl-10"
          />
              ...
              <div className="flex items-center space-x-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <Receipt className="h-3 w-3 text-blue-500" />
                    <span className="text-xs text-blue-600 font-medium">
                      {getTodayChecksCount(expeditor.name)} bugungi cheklar
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 border-b border-gray-200">
                ...
                {selectedExpeditor && (
                  <div className="relative">
                    <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-4 w-4" />
                    <Input
                      placeholder="Cheklarni qidirish..."
                      value={checkSearchQuery}
                      onChange={(e) => setCheckSearchQuery(e.target.value)}
                      className="pl-10"
                    />
                        ...
                        <CardContent className="p-4">
                          <div className="space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              <Badge variant="outline" className="text-xs">
                                {new Date(check.check_date).toLocaleDateString("uz-UZ", {
                                  day: "2-digit",
                                  month: "2-digit",
                                  year: "numeric",
                                })}
                            ...
                            <div className="text-xs text-gray-600 space-y-1">
                              <p>
                                <strong>Loyiha:</strong> {check.project}
                              </p>
                              <p>
                                <strong>Shahar:</strong> {check.city}
                              </p>
                              <p>
                                <strong>KKM:</strong> {check.kkm_number}
                              </p>
